package com.application.constants;

public class FileConstants {

    public static final String USER_IMAGE_PATH = "api/user/image/";
    public static final String HOTEL_IMAGE_PATH = "api/hotel/image/";
    public static final String ROOM_IMAGE_PATH = "api/room/image/";
    public static final String JPG_EXTENSION = "jpg";
    public static final String USER_FOLDER = System.getProperty("user.home") + "/PLAN-PRO-IMAGES/images/";
    public static final String ROOM_FOLDER = System.getProperty("user.home") + "/HOTEL-BOOKING-APPLICATION/images/rooms/";
    public static final String DIRECTORY_CREATED = "Created directory for: ";
    public static final String DEFAULT_USER_IMAGE_PATH = "api/user/image/profile/";
    public static final String DEFAULT_HOTEL_IMAGE_PATH = "api/hotel/image/profile/";
    public static final String DEFAULT_ROOM_IMAGE_PATH = "api/user/image/profile/";
    public static final String FILE_SAVED_IN_FILE_SYSTEM = "Saved file in file system by name: ";
    public static final String DOT = ".";
    public static final String FORWARD_SLASH = "/";
    public static final String NOT_AN_IMAGE_FILE = " is not an image file. Please upload an image file";
    public static final String TEMP_PROFILE_IMAGE_BASE_URL = "https://robohash.org/";

}


